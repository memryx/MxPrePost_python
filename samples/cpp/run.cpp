...
static void print_usage(const char* prog) {
    std::cout << "Usage: " << prog
              ...
              << "Examples:\n"
              << "  " << prog
              << " -d model.dfp -t yolov8_det --video_paths /dev/video0 /dev/video2\n"
              << "  " << prog << " -d model.dfp -t yolov8_det --video_paths video.mp4 --no-show\n";
}
  ...
  private:
    ...
    bool in_callback(std::vector<const MX::Types::FeatureMap*> dst, int stream_id) {
        while (true) {
            ...
            // Preprocess
            cv::Mat pre = prepost_->preprocess(frame);
