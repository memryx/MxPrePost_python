...
TEST(NMS, SingleBox) {
    std::vector<BBox> boxes = {BBox(0.0f, 0.0f, 10.0f, 10.0f, 0.8f, 0, "")};
    std::vector<int> result = nms(boxes, 0.5f, false);
...
TEST(NMS, PerfectOverlap) {
    ...
    std::vector<BBox> boxes = {
            ...
            BBox(0.0f, 0.0f, 10.0f, 10.0f, 0.5f, 0, "")   // boxes[1] - class 0, identical
                                                          // coordinates
    };
    std::vector<int> result = nms(boxes, 0.5f, false);
...
TEST(NMS, IoUThresholdBoundaries) {
    ...
    // Test 1: IoU below threshold (0.3 < 0.5) - both should be kept
    {
        ...
        EXPECT_EQ(result.size(), 2)
                << "Both boxes should be kept when IoU (0.333) < threshold (0.5)";
        EXPECT_EQ(result[0], 0);
    ...
    // Test 2: IoU exactly equal to threshold (0.5 = 0.5) - both should be kept (strict > check)
    {
        ...
        std::vector<BBox> boxes = {
                ...
                BBox(0.0f, 0.0f, 20.0f, 10.0f, 0.5f, 1, "")  // boxes[1] - area = 200, intersection
                                                             // = 100
        };
        std::vector<int> result = nms(boxes, 0.5f, false);
        EXPECT_EQ(result.size(), 2)
                << "Both boxes should be kept when IoU (0.5) == threshold (0.5) due to strict > check";
        EXPECT_EQ(result[0], 0);
        EXPECT_EQ(result[1], 1);
    }
    ...
    // Test 3: IoU above threshold (0.6 > 0.5) - lower conf should be suppressed (same class)
    {
        ...
        std::vector<BBox> boxes = {
                ...
                BBox(0.0f, 0.0f, 10.0f, 10.0f, 0.5f, 0, "")   // boxes[1] - class 0, lower conf,
                                                              // perfect overlap
        };
        std::vector<int> result = nms(boxes, 0.5f, false);
        EXPECT_EQ(result.size(), 1)
                << "Lower confidence box should be suppressed when IoU (1.0) > threshold (0.5)";
        EXPECT_EQ(result[0], 0);
    }
}
...
TEST(NMS, MultipleOverlaps) {
    ...
    std::vector<BBox> boxes = {
            BBox(0.0f, 0.0f, 10.0f, 10.0f, 0.9f, 0, ""),  // boxes[0] - class 0, highest conf, area
                                                          // = 100
...
TEST(NMS, TieConfidence) {
    ...
    std::vector<BBox> boxes = {
            ...
            BBox(0.0f, 0.0f, 10.0f, 10.0f, 0.8f, 0, "")   // boxes[1] - class 0, conf = 0.8 (tie),
                                                          // perfect overlap
    };
    std::vector<int> result = nms(boxes, 0.5f, false);
...
TEST(NMS, ClassAgnosticSameConfidence) {
    ...
    std::vector<BBox> boxes = {
            ...
            BBox(0.0f, 0.0f, 10.0f, 10.0f, 0.8f, 1, "car")      // boxes[1] - class 1, conf = 0.8
                                                                // (tie), perfect overlap
    };
...
TEST(NMS, ClassAgnosticDifferentConfidence) {
    ...
    std::vector<BBox> boxes = {
            ...
            BBox(0.0f, 0.0f, 10.0f, 10.0f, 0.5f, 1, "car")      // boxes[1] - class 1, conf = 0.5,
                                                                // perfect overlap
    };
    std::vector<int> result = nms(boxes, 0.5f, true);
...
TEST(Preprocess, BasicFunctionality) {
    ...
    cv::Mat result =
            preprocess(input, letterbox_w, letterbox_h, pad_left, pad_top, pad_right, pad_bottom);

    EXPECT_EQ(result.rows, letterbox_h + pad_top + pad_bottom);  // 360 + 140 + 140 = 640
    EXPECT_EQ(result.cols, letterbox_w + pad_left + pad_right);  // 640 + 0 + 0 = 640
    EXPECT_EQ(result.type(), CV_32FC3);

    // Verify value range [0.0, 1.0]
    double min_val, max_val;
    cv::minMaxLoc(result.reshape(1), &min_val, &max_val);  // reshape so min/max scans all channels
    EXPECT_GE(min_val, 0.0);
    EXPECT_LE(max_val, 1.0);

    // Verify top padding is zero (rows 0..pad_top-1)
    cv::Mat top_padding = result(cv::Rect(0, 0, result.cols, pad_top));
    cv::Scalar top_sum = cv::sum(top_padding);
    EXPECT_NEAR(top_sum[0] + top_sum[1] + top_sum[2], 0.0, 1e-5);

    // Verify bottom padding is zero (last pad_bottom rows)
    int bottom_start_row = pad_top + letterbox_h;  // 140 + 360 = 500
    cv::Mat bottom_padding = result(cv::Rect(0, bottom_start_row, result.cols, pad_bottom));
    cv::Scalar bottom_sum = cv::sum(bottom_padding);
    EXPECT_NEAR(bottom_sum[0] + bottom_sum[1] + bottom_sum[2], 0.0, 1e-5);

    // Verify left/right edges in the content region are not padding (since pad_left/right = 0)
    cv::Mat left_edge = result(cv::Rect(0, pad_top, 1, letterbox_h));
    cv::Scalar left_edge_mean = cv::mean(left_edge);
    EXPECT_GT(left_edge_mean[0] + left_edge_mean[1] + left_edge_mean[2], 0.0);

    cv::Mat right_edge = result(cv::Rect(result.cols - 1, pad_top, 1, letterbox_h));
    cv::Scalar right_edge_mean = cv::mean(right_edge);
    EXPECT_GT(right_edge_mean[0] + right_edge_mean[1] + right_edge_mean[2], 0.0);

    // Verify center region is normalized correctly
    cv::Mat center = result(cv::Rect(pad_left, pad_top, letterbox_w, letterbox_h));
    cv::Scalar center_mean = cv::mean(center);
    float expected = 128.0f / 255.0f;
    EXPECT_NEAR(center_mean[0], expected, 1e-3f);
    EXPECT_NEAR(center_mean[1], expected, 1e-3f);
    EXPECT_NEAR(center_mean[2], expected, 1e-3f);
}

TEST(Preprocess, SquareImageNoPadding) {
    cv::Mat input(640, 640, CV_8UC3, cv::Scalar(255, 255, 255));

    cv::Mat result = preprocess(input, 640, 640, 0, 0, 0, 0);
    ...
    EXPECT_NEAR(mean_val[1], 1.0f, 1e-5f);
    EXPECT_NEAR(mean_val[2], 1.0f, 1e-5f);
}
...
TEST(Preprocess, WideImageVerticalPadding) {
    ...
    EXPECT_EQ(result.rows, letterbox_h + pad_top + pad_bottom);  // 160 + 240 + 240 = 640
    EXPECT_EQ(result.cols, letterbox_w);                         // 640

    // Verify top padding is zero
    cv::Mat top_pad = result(cv::Rect(0, 0, result.cols, pad_top));
    cv::Scalar top_sum = cv::sum(top_pad);
    EXPECT_NEAR(top_sum[0] + top_sum[1] + top_sum[2], 0.0, 1e-5);

    // Verify bottom padding is zero
    int bottom_start_row = pad_top + letterbox_h;  // 240 + 160 = 400
    cv::Mat bottom_pad = result(cv::Rect(0, bottom_start_row, result.cols, pad_bottom));
    cv::Scalar bottom_sum = cv::sum(bottom_pad);
    EXPECT_NEAR(bottom_sum[0] + bottom_sum[1] + bottom_sum[2], 0.0, 1e-5);

    // Verify center content is normalized
    cv::Mat center = result(cv::Rect(0, pad_top, letterbox_w, letterbox_h));
    cv::Scalar center_mean = cv::mean(center);
    float expected = 200.0f / 255.0f;
    EXPECT_NEAR(center_mean[0], expected, 1e-3f);
    EXPECT_NEAR(center_mean[1], expected, 1e-3f);
    EXPECT_NEAR(center_mean[2], expected, 1e-3f);
}
...
TEST(Preprocess, TallImageHorizontalPadding) {
    ...
    EXPECT_EQ(result.rows, letterbox_h);                         // 640
    EXPECT_EQ(result.cols, letterbox_w + pad_left + pad_right);  // 160 + 240 + 240 = 640
    EXPECT_EQ(result.type(), CV_32FC3);

    // Verify left padding is zero
    cv::Mat left_pad = result(cv::Rect(0, 0, pad_left, result.rows));
    cv::Scalar left_sum = cv::sum(left_pad);
    EXPECT_NEAR(left_sum[0] + left_sum[1] + left_sum[2], 0.0, 1e-5);

    // Verify right padding is zero
    int right_start_col = pad_left + letterbox_w;  // 240 + 160 = 400
    cv::Mat right_pad = result(cv::Rect(right_start_col, 0, pad_right, result.rows));
    cv::Scalar right_sum = cv::sum(right_pad);
    EXPECT_NEAR(right_sum[0] + right_sum[1] + right_sum[2], 0.0, 1e-5);

    // Verify center content is normalized
    cv::Mat center = result(cv::Rect(pad_left, 0, letterbox_w, letterbox_h));
    cv::Scalar center_mean = cv::mean(center);
    float expected = 100.0f / 255.0f;
    EXPECT_NEAR(center_mean[0], expected, 1e-3f);
    EXPECT_NEAR(center_mean[1], expected, 1e-3f);
    EXPECT_NEAR(center_mean[2], expected, 1e-3f);
}
...
TEST(Preprocess, VerySmallImage) {
    ...
    cv::Scalar mean_val = cv::mean(result);
    float expected = 50.0f / 255.0f;
    EXPECT_NEAR(mean_val[0], expected, 1e-3f);
    EXPECT_NEAR(mean_val[1], expected, 1e-3f);
    EXPECT_NEAR(mean_val[2], expected, 1e-3f);

    // Verify no padding (all values should be > 0)
    double min_val;
    cv::minMaxLoc(result.reshape(1), &min_val, nullptr);
...
TEST(Preprocess, UnevenVerticalPadding) {
    // Input is 640x315 (width x height)
    cv::Mat input(315, 640, CV_8UC3, cv::Scalar(150, 150, 150));

    int letterbox_w = 640;
    int letterbox_h = 315;

    // Total padding required = 640 - 315 = 325
    // Must split unevenly: 162 + 163
    int pad_top = 162;
    int pad_bottom = 163;
    int pad_left = 0;
    int pad_right = 0;
    ...
    // 🔴 This is the critical check that previously failed (639 bug)
    EXPECT_EQ(result.rows, 640);
    EXPECT_EQ(result.cols, 640);
    EXPECT_EQ(result.type(), CV_32FC3);

    // Verify top padding is zero
    cv::Mat top_pad = result(cv::Rect(0, 0, 640, pad_top));
    cv::Scalar top_sum = cv::sum(top_pad);
    EXPECT_NEAR(top_sum[0] + top_sum[1] + top_sum[2], 0.0, 1e-5);

    // Verify bottom padding is zero
    cv::Mat bottom_pad = result(cv::Rect(0, pad_top + letterbox_h, 640, pad_bottom));
    cv::Scalar bottom_sum = cv::sum(bottom_pad);
    EXPECT_NEAR(bottom_sum[0] + bottom_sum[1] + bottom_sum[2], 0.0, 1e-5);

    // Verify center region contains normalized content
    cv::Mat center = result(cv::Rect(0, pad_top, letterbox_w, letterbox_h));
    cv::Scalar center_mean = cv::mean(center);

    float expected = 150.0f / 255.0f;
    EXPECT_NEAR(center_mean[0], expected, 1e-3f);
    EXPECT_NEAR(center_mean[1], expected, 1e-3f);
    EXPECT_NEAR(center_mean[2], expected, 1e-3f);

    // Ensure no accidental zero row inside content region
    double min_val;
    cv::minMaxLoc(center.reshape(1), &min_val, nullptr);
    EXPECT_GT(min_val, 0.0);
}
...
TEST(DrawMask, PolygonAndBoundingBoxMatchInput) {
    ...
    cv::Mat bbox_top = image(cv::Rect(expected_rect.x, expected_rect.y, expected_rect.width, 1));
